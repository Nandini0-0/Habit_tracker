class habit_obj {
    constructor(i) {
        ...
        let selector_class = document.querySelector('.regular-habits').querySelectorAll('input[type="checkbox"]')[i];class habit_obj {
    constructor(i) {
        let selector_name = document.querySelector('.regular-habits').querySelectorAll('span');
        this.habit_name = selector_name[i].innerHTML;
...
document.querySelector(".hero").innerHTML += `<p>${today.toDateString()}</p>`;
...
function update_progress() {
    ...
    progress_fill_style.backgroundColor = `hsl(${hue},80%,50%)`;
    if(total_tasks()){
        document.querySelector(".task_prog").innerHTML = `Today's tasks completed - ${completed_tasks()}/${total_tasks()}`
    }
    else{
    document.querySelector(".task_prog").innerHTML = `No task on the list!`
}
if(total_habits()){
    document.querySelector(".habit_prog").innerHTML = `Regular habits completed - ${completed_habits()}/${total_habits()}`
}
else{
        document.querySelector(".habit_prog").innerHTML = `No habits added!`
